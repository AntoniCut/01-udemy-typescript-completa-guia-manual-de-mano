...
export const spaLoaderContentHtml = (options = {}) => {
     ...
     * ----------------------------------
     * 
     * Comprueba si ya existe un stylesheet cargado (previene duplicados)
     * 
    ...
    const isStylesheetLoaded = (cssFile) => {

        /**
         * - `Lista de hojas de estilo actualmente cargadas en el documento`
        ...
        return links.some(l => {
            ...
            return href.split('?')[0].endsWith(cssFile);
        });
    }
    ...
    const loadStylesheet = (cssFile) => {
        ...
        //  -----  prevenir duplicados  -----
        if (isStylesheetLoaded(cssFile)) {
            console.log(`🔁 Stylesheet ya cargado: ${cssFile}`);
            return;
        }


        /**
         * - `Href con timestamp para evitar cache` 
         * @type {string}
         */
        ...
        //  -----  eliminar styles previos marcados como page-style  -----
        document.querySelectorAll('link[data-page-style="true"]').forEach(link => link.remove());

        //  -----  crear y añadir nuevo link  -----

        /**
         * - `Elemento link para la hoja de estilos` 
         * @type {HTMLLinkElement}
         */
